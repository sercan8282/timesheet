...
db.serialize(() => {
  ...
  db.run(
    "UPDATE leave_balances SET vacation_hours = 0, overtime_hours = 0",
    (err) => {
      if (err) {
  ...
  db.get("SELECT COUNT(*) as count FROM submissions", (err, row) => {
    console.log(
      `Verification - Submissions remaining: ${row ? row.count : "error"}`
    );
  });

  db.get("SELECT COUNT(*) as count FROM planning_schedules", (err, row) => {
    console.log(
      `Verification - Planning schedules remaining: ${
        row ? row.count : "error"
      }`
    );
  });
  ...
  db.get("SELECT COUNT(*) as count FROM leave_requests", (err, row) => {
    console.log(
      `Verification - Leave requests remaining: ${row ? row.count : "error"}`
    );

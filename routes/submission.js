...
router.post(
  '/submit',
  [
    body('timesheetIds').isArray({ min: 1 }).withMessage('At least one timesheet must be selected')
  ],
  async (req, res) => {
    try {
      ...
      if (emailStatus === 'sent') {
        res.json({ message: 'Timesheets submitted and email sent successfully' });
      } else {
        res.json({ 
          message: 'Timesheets submitted but email failed to send. Please check SMTP settings.',
          warning: emailError
        });
      }
    } catch (error) {
      console.error('Error submitting timesheets:', error);
      res.status(500).json({ error: 'Failed to submit timesheets: ' + error.message });
    }
  }
);
...
router.post('/preview-pdf', async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `inline; filename=timesheet_preview.pdf`);
    res.send(pdfBuffer);
  } catch (error) {
...
router.post('/preview-xlsx', async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=timesheet_preview.xlsx`);
    res.send(xlsxBuffer);
  } catch (error) {
...
router.post('/submissions/:id/resend', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to resend submission: ' + error.message });
  }
});
